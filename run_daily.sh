...
# 開始メッセージ
echo "===== ArXivTweetBot 日次実行 =====" >> "$EXEC_LOG"
...
# LLMとRAGのキーワードで検索して処理（デフォルトで最大4件）
echo "LLMとRAGのキーワードで検索を実行中..." >> "$EXEC_LOG"
$PYTHON_CMD "$SCRIPT_DIR/arxiv_downloader.py" "LLM" "RAG" >> "$EXEC_LOG" 2>&1

# 実行結果のサマリーを生成
echo "" >> "$EXEC_LOG"
echo "Twitter投稿ログの分析中..." >> "$EXEC_LOG"
$PYTHON_CMD "$SCRIPT_DIR/twitter_log_analyzer.py" --output "$LOG_DIR/twitter_summary_$DATE.md" --csv "$LOG_DIR/twitter_posts_$DATE.csv" >> "$EXEC_LOG" 2>&1

# 完了メッセージ
echo "" >> "$EXEC_LOG"
echo "処理が完了しました。" >> "$EXEC_LOG"
echo "サマリー: $LOG_DIR/twitter_summary_$DATE.md" >> "$EXEC_LOG"
echo "CSVデータ: $LOG_DIR/twitter_posts_$DATE.csv" >> "$EXEC_LOG"
echo "===== 終了 =====" >> "$EXEC_LOG"